...
from build_staging_workflow import FAMILY_SHEETS, sha256_file
...
def build():
    ...
    route_block={'id':'10310000-0000-4000-8000-000000000003','name':'WU103 Routing Guard Blocked','type':'n8n-nodes-base.code','typeVersion':2,'position':[720,560],'parameters':{'jsCode':ROUTING_BLOCK_JS}}
    wf['nodes'].extend([pre,gate,route_block])
    ...
    c['Load WU103 KB Shadow [STAGING]']={'main':[[{'node':family_ifs[0]['name'],'type':'main','index':0}]]}
...
def main():
    ...
    print(json.dumps({'output':str(out),'sha256':sha256_file(out),'nodes':len(wf['nodes']),'connections':len(wf['connections']),'active':wf['active'],'memory_safety':'single-candidate+single-family+exact-id-read'},indent=2))
