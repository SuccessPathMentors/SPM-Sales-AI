...
LOGGER = 'Upsert WU101 Analytics [STAGING]'
...
def build(baseline_path):
    ...
    if not isinstance(doc, dict) or not doc.get('__rl'):
        raise RuntimeError('unexpected WU-101 analytics documentId shape')
    ...
    if logger.get('onError') != 'continueRegularOutput':
        raise RuntimeError('analytics logger is not configured fail-open')
    return wf
...
def main():
    ...
    print(json.dumps({
        ...
        'failure_injection': 'analytics_document_not_found',
